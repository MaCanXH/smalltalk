...
- **`supabase/functions/api/`** — the backend, a **Supabase Edge Function** (Deno) named `api`, deployed with `npx supabase functions deploy api`. One function routes all endpoints internally: `POST .../functions/v1/api/feedback` (the critic), `GET .../functions/v1/api/hot-topics` (the news topic packs consumed by `lib/news/hotTopics.ts`), and `POST .../functions/v1/api/vapi/session` (per-call Vapi config, see above). Reads its secrets from **Supabase secrets** — never shipped to the client. JWT verification is on (`verify_jwt` in `supabase/config.toml`): requests must carry a Supabase JWT, which the app adds via `getFunctionsAuthHeaders()` in `lib/supabase.ts` (the signed-in user's access token; `/api/vapi/session` additionally rejects the bare anon key). The single-function layout is deliberate — the app derives the hot-topics URL from `EXPO_PUBLIC_FEEDBACK_API_URL` by stripping `/api/feedback`, and both routes living under `/functions/v1/api/` keeps that working. Local dev: `npx supabase start` then `npx supabase functions serve api --env-file .env --no-verify-jwt` (needs Docker). The function's Deno code is excluded from `tsc` (tsconfig `exclude`) and ESLint (flat-config `ignores`); keep the response JSON shapes in sync with the client's `normalizeTopic` / `validateAiFeedback`. (The earlier local Express backend, `server.mjs`, has been removed.)
...
**Server-side secrets** (Supabase secrets, never in the client bundle): `GROQ_API_KEY` (critic + topic packs), `VAPI_ASSISTANT_ID`, `VAPI_PRIVATE_KEY` + `VAPI_ORG_ID` (sign the short-lived per-call Vapi JWTs; both from the Vapi dashboard), `VAPI_PUBLIC_KEY` (legacy fallback token until the private key/org id are set), optional `VAPI_DAILY_SESSION_LIMIT`. Set with `npx supabase secrets set NAME=value`. `GROQ_API_KEY` also sits in local `.env` solely for `supabase functions serve`.

**Sign-in is required.** There is no "continue offline" path — the sign-in gate (`app/_layout.tsx` → `components/SignInScreen.tsx`) blocks the tabs until a Supabase session exists, because `/api/vapi/session` attributes and rate-limits voice sessions per user. The `vapi_call_grants` quota table is part of `supabase/schema.sql` (manual dashboard setup, like the other tables).
