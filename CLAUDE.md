...
**Optional (AI critic + hot topics):** `EXPO_PUBLIC_FEEDBACK_API_URL` points the app at the deployed Edge Function (`https://<project-ref>.supabase.co/functions/v1/api/feedback` — same URL for emulator and physical device). Without it, results fall back to deterministic scoring and the Talk tab to the offline topic catalog. `GROQ_API_KEY` lives in **Supabase secrets** (and in local `.env` only for `supabase functions serve`) — it has no `EXPO_PUBLIC_` prefix on purpose and must never reach the client.
